...
export default function MyRecipeScreen() {
  ...
  useEffect(() => {
    const fetchrecipes = async () => {
      try {
        // Fetch the recipes from AsyncStorage
        const storedRecipes = await AsyncStorage.getItem("customrecipes");

        if (storedRecipes) {
          // Parse and set the recipes to state
          const parsedRecipes = JSON.parse(storedRecipes);
          setrecipes(parsedRecipes);
        } else {
          // If no recipes are found, set an empty array
          setrecipes([]);
        }
      } catch (error) {
        console.error("Error fetching recipes:", error);
      } finally {
        // Update the loading state to indicate loading is complete
        setLoading(false);
      }
    };
  ...
  const deleterecipe = async (index) => {
    try {
      ...
      // Update AsyncStorage with the updated recipes array
            ...
            recipes.map((recipe, index) => (
              <View key={index} style={styles.recipeCard} testID="recipeCard">
                ...
                >
                  ...
                  <TouchableOpacity style={{backgroundColor: 'black',}} onPress={() => deleterecipe(index)}>
